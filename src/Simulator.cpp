...
namespace simulator {
  ...
  */
  void Simulator::init() {
    ...
    GLFWwindow* window = glfwCreateWindow(mode->width, mode->height,
                                          "Swarm Simulator", monitor, NULL);
    glfwSetWindowMonitor(window, NULL, 1, 1, mode->width, mode->height,
                        mode->refreshRate);
    ...
    glfwSetWindowUserPointer(window,
                            reinterpret_cast<void*>(AdditionalParameters));
  }
  ...
  */
  void Simulator::run(OpenglBuffersManager* openglBuffersManager) {
    // Main rendering loop
      ...
      switch (userInterface->stateSimulation) {
        case RESET: {
          ...
          while (RESET == userInterface->stateSimulation) {
            ...
            switch (userInterface->UIAction) {
              ...
              case ADD_ANT: {
                environment->createAnt(userInterface->nestID,
        ...
        case RUNNING: {
          ...
          while (userInterface->stateSimulation == RUNNING) {
            ...
            userInterface->run();  // RETIRAR DAQUI PARA MAIOR EXCLUSIVIDADE DO
                                  // RUN
            post_render();
          }  // while loop
  ...
  static void keyCallback(GLFWwindow* window, int key, int scancode, int action,
    ...
    AdditionalCallbackParameters* AdditionalParameters =
        reinterpret_cast<AdditionalCallbackParameters*>(
